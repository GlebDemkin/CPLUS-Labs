...
void MenuSystem::drawCurrentMenu(NVGcontext* context, float screenWidth, float screenHeight) {
    ...
    nvgText(context, screenWidth / 2.0f, 80.0f, currentMenu.getText().c_str(), nullptr);
    ...
    // Считаем общую высоту всех кнопок,
    // потому что теперь каждая кнопка может иметь свой размер из menu.gleb.
    float totalHeight = 0.0f;

    for (std::size_t i = 0; i < count; ++i) {
        totalHeight += currentMenu.getChild(i).getHeight();
    }

    totalHeight += gap * static_cast<float>(count - 1);
